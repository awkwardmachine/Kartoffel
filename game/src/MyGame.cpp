...
void MyGame::OnInit() {
    ...
    const unsigned int shared_shader =
            AssetManager::GetShader("assets/shaders/triangle.vert", "assets/shaders/triangle.frag");
    ...
    GetWorld().AddComponent<ScriptComponent>(
            e, ScriptComponent{
                       .on_init = nullptr,
                       .on_update =
                               [timer = 0.0f, dropped = false](const Entity self, World &w, const float dt) mutable {
                                   if (dropped)
                                       return;
                                   timer += dt;
                                   if (timer >= 3.0f) {
