...
namespace {
    ...
    std::string V3(const glm::vec3 &v) { return P(v.x) + " " + P(v.y) + " " + P(v.z); }
...
bool SceneLoader::Load(const std::string &path, World &world) {
    ...
    while (std::getline(file, raw)) {
        ...
        if (block == BlockType::Entity) {
            if (key == "name" && tok.size() >= 2) {
                cur_entity.name = tok[1];
            } else if (key == "tags") {
                for (size_t i = 1; i < tok.size(); ++i)
                    cur_entity.tags.push_back(tok[i]);
            } else if (key == "model" && tok.size() >= 2) {
                cur_entity.kasset_path = tok[1];
                cur_entity.has_model   = true;
            } else if (key == "shader" && tok.size() >= 3) {
                cur_entity.vert_shader = tok[1];
                cur_entity.frag_shader = tok[2];
            } else if (key == "position" && tok.size() >= 4) {
                cur_entity.position = {ToFloat(tok[1]), ToFloat(tok[2]), ToFloat(tok[3])};
            } else if (key == "rotation" && tok.size() >= 4) {
                cur_entity.rotation = {ToFloat(tok[1]), ToFloat(tok[2]), ToFloat(tok[3])};
            } else if (key == "scale" && tok.size() >= 4) {
                cur_entity.scale = {ToFloat(tok[1]), ToFloat(tok[2]), ToFloat(tok[3])};
            }
            // rigidbody fields
            else if (key == "rigidbody") {
                cur_entity.has_rigidbody = true;
            } else if (key == "mass" && tok.size() >= 2) {
                cur_entity.mass          = ToFloat(tok[1]);
                cur_entity.has_rigidbody = true;
            } else if (key == "restitution" && tok.size() >= 2) {
                cur_entity.restitution = ToFloat(tok[1]);
            } else if (key == "linear_drag" && tok.size() >= 2) {
                cur_entity.linear_drag = ToFloat(tok[1]);
            } else if (key == "use_gravity" && tok.size() >= 2) {
                cur_entity.use_gravity = (tok[1] == "true" || tok[1] == "1");
            } else if (key == "is_kinematic" && tok.size() >= 2) {
                cur_entity.is_kinematic = (tok[1] == "true" || tok[1] == "1");
            } else if (key == "velocity" && tok.size() >= 4) {
                cur_entity.velocity = {ToFloat(tok[1]), ToFloat(tok[2]), ToFloat(tok[3])};
            }
            // box collider fields
            else if (key == "collider") {
                cur_entity.has_collider = true;
            } else if (key == "half_extents" && tok.size() >= 4) {
                cur_entity.half_extents = {ToFloat(tok[1]), ToFloat(tok[2]), ToFloat(tok[3])};
                cur_entity.has_collider = true;
            } else if (key == "col_offset" && tok.size() >= 4) {
                cur_entity.col_offset = {ToFloat(tok[1]), ToFloat(tok[2]), ToFloat(tok[3])};
            } else if (key == "is_trigger" && tok.size() >= 2) {
                cur_entity.is_trigger = (tok[1] == "true" || tok[1] == "1");
            } else if (key == "col_layer" && tok.size() >= 2) {
                cur_entity.col_layer = static_cast<CollisionLayer>(std::stoul(tok[1]));
            } else if (key == "col_mask" && tok.size() >= 2) {
                cur_entity.col_mask = static_cast<CollisionLayer>(std::stoul(tok[1]));
            } else {
                std::cerr << "[SceneLoader] " << path << ":" << line_num << " unknown entity field '" << key << "'"
                          << std::endl;
            }
        } else if (block == BlockType::Light) {
            if (key == "name" && tok.size() >= 2) {
                cur_light.name = tok[1];
            } else if (key == "tags") {
                for (size_t i = 1; i < tok.size(); ++i)
                    cur_light.tags.push_back(tok[i]);
            } else if (key == "type" && tok.size() >= 2) {
                cur_light.type = ParseLightType(tok[1]);
            } else if (key == "color" && tok.size() >= 4) {
                cur_light.color = {ToFloat(tok[1]), ToFloat(tok[2]), ToFloat(tok[3])};
            } else if (key == "intensity" && tok.size() >= 2) {
                cur_light.intensity = ToFloat(tok[1]);
            } else if (key == "position" && tok.size() >= 4) {
                cur_light.position = {ToFloat(tok[1]), ToFloat(tok[2]), ToFloat(tok[3])};
            } else if (key == "direction" && tok.size() >= 4) {
                cur_light.direction = {ToFloat(tok[1]), ToFloat(tok[2]), ToFloat(tok[3])};
            } else if (key == "constant" && tok.size() >= 2) {
                cur_light.constant = ToFloat(tok[1]);
            } else if (key == "linear" && tok.size() >= 2) {
                cur_light.linear = ToFloat(tok[1]);
            } else if (key == "quadratic" && tok.size() >= 2) {
                cur_light.quadratic = ToFloat(tok[1]);
            } else if (key == "inner_cutoff" && tok.size() >= 2) {
                cur_light.inner_cutoff_deg = ToFloat(tok[1]);
            } else if (key == "outer_cutoff" && tok.size() >= 2) {
                cur_light.outer_cutoff_deg = ToFloat(tok[1]);
            } else {
                std::cerr << "[SceneLoader] " << path << ":" << line_num << " unknown light field '" << key
                          << std::endl;
            }
        }
    }
    ...
    for (const auto &ed: entities) {
        ...
        for (const auto &[mesh, texture]: meshes) {
            ...
            world.AddComponent<TransformComponent>(
                    e, TransformComponent{.position = ed.position, .rotation = rot_rad, .scale = ed.scale});
            world.AddComponent<MeshComponent>(e, mesh);
            world.AddComponent<ShaderComponent>(e, ShaderComponent{.id = shader_id});
            world.AddComponent<ModelComponent>(e, ModelComponent{.kasset_path = ed.kasset_path,
                                                                 .vert_shader = ed.vert_shader,
                                                                 .frag_shader = ed.frag_shader});
    ...
    for (const auto &[name, tags, type, color, intensity, position, direction, constant, linear, quadratic,
...
bool SceneLoader::Save(const std::string &path, const World &world) {
    ...
    for (const Entity e: world.Query<ModelComponent, TransformComponent>()) {
        ...
        if (!first_model && kasset_path == last_model.kasset_path && position == last_model.position &&
            rotation == last_model.rotation)
            continue;
